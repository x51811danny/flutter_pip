...
class _MyAppState extends State<MyApp> with WidgetsBindingObserver, TickerProviderStateMixin {
      ...
      switch (call.method) {
        ...
        case 'onVideoFullScreenClick':
          ...
          if (routeNames.last != 'testFlutterVideoView') {
            NavigationUtil.getInstance().pushPage(context, "testFlutterVideoView",
                widget: TestFlutterVideoViewPage(
                  isFromPip: true,
                ));
          }
          break;
  ...
  void initState() {
    ...
    _animationScale = Tween(begin: 1.0, end: scaleFactor).animate(_animationController)
      ..addListener(() {
        setState(() {});
        overlayEntryX.markNeedsBuild();
      });
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    switch (state) {
      ...
      case AppLifecycleState.paused:
        print("paused");
        Map<String, String> params = {"videoUrl": 'http://vfx.mtime.cn/Video/2019/03/18/mp4/190318231014076505.mp4'};
                  ...
                  Platform.isAndroid
                      ? Wrap(
                          ...
                          children: [
                              ...
                              children: [
                                ...
                                ElevatedButton(
                                    onPressed: () {
                                      ...
                                      var map = {"width": int.parse(width), "height": int.parse(height)};
                            ...
                            ElevatedButton(
                                onPressed: () {
                                  FlutterFloatWindow.setNotificationChannelIdAndName('234567', '画中画通知');
                                  FlutterFloatWindow.showPlaybackNotification("qeubee", "你猜猜~~~~~");
                                },
        ...
        child: FittedBox(
          ...
          child: Row(
            children: [
              Text(
                'hello\t\t\t\t\t',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 30, color: Colors.grey.withOpacity(0.1)),
              ),
              Text(
                'hello\t\t\t\t\t',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 30, color: Colors.grey.withOpacity(0.1)),
